...
// Vite plugin that mounts the Netlify-style serverless functions as API routes
// during development. In Vite 5, middleware must be added via configureServer,
// not via a `server.middleware` config key (which doesn't exist).
function apiPlugin(routes) {
  return {
    name: 'synoma-api',
...
export default defineConfig(async ({ mode }) => {
  ...
  return {
    plugins: mode === 'serve' ? [apiPlugin(routes)] : [],
    server: {
      port: 5173,
    },
    build: {
